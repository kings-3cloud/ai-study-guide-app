...
const modeMultiAgentBtn  = document.getElementById('modeMultiAgentBtn');
...
// Mode toggle buttons
[modeOnlineBtn, modeMultiAgentBtn].forEach(btn => {
    ...
    fetch('/mode', {
        ...
        .then(data => {
            if (data.error) {
                ...
                if (data.changed) {
                    const modeLabels = {
                        'online':      '☁️ Online (Foundry)',
                        'multi-agent': '🤖 Multi-Agent (Foundry — Content · Quiz · Progress)',
                    };
                    const label = modeLabels[data.mode] || data.mode;
